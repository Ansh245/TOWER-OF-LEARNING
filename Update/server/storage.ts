...
export class DatabaseStorage implements IStorage {
  ...
  async seedInitialData(): Promise<void> {
    ...
    const existingLectures = await db.select().from(lectures).limit(1);
  ...
  private generateQuestionsForLecture(lectureId: string, title: string): InsertQuizQuestion[] {
    ...
    return questionSets[title] || [
      { lectureId, question: `What is the main topic of ${title}?`, options: ["Unrelated concept", title, "Programming basics", "Theory only"], correctAnswer: 1, difficulty: baseDifficulty as const, timeLimit: 30 },
      { lectureId, question: `Why is ${title} important?`, options: ["It isn't", "Foundation for further learning", "Rarely used", "Outdated"], correctAnswer: 1, difficulty: baseDifficulty as const, timeLimit: 30 },
      { lectureId, question: `${title} helps you understand:`, options: ["Unrelated skills", "Core programming concepts", "Only theory", "Nothing practical"], correctAnswer: 1, difficulty: baseDifficulty as const, timeLimit: 30 },
    ];
  }
}
