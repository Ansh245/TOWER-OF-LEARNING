...
export function calculateLevel(totalXp: number): number {
  ...
  while (xpAccumulated + xpRequired <= totalXp) {
    ...
    console.log(`  Level ${level}: accumulated=${xpAccumulated}, required for next=${xpRequired}`);
  }
  
  console.log(`📊 FINAL: totalXp=${totalXp} → level=${level}`);
  return level;
}
...
// Multiplier for XP/rewards based on difficulty
export function getDifficultyMultiplier(difficulty: "easy" | "medium" | "hard") {
  switch (difficulty) {
    case "easy":
      return 0.8;
    case "medium":
